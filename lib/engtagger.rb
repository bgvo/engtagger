...
class EngTagger
  ...
  def get_determiners(tagged)
    [get_pure_determiners(tagged), get_prp_determiners(tagged), get_prps_determiners(tagged), get_pdt_determiners(tagged), get_wdt_determiners(tagged), get_wps_determiners(tagged)].reduce &:merge
  end

  def get_pronouns(tagged)
    [get_ex_pronoun(tagged), get_wp_pronoun(tagged)].reduce &:merge
  end

  def get_prepositions(tagged)
    [get_in_preposition(tagged), get_to_preposition(tagged)].reduce &:merge
  end

  def get_interjections(tagged)
    get_uh_interjection(tagged)
  end

  def get_uh_interjection(tagged)
    return nil unless valid_text(tagged)
    UH
    trimmed = tagged.scan(UH).map do |n|
      strip_tags(n)
    end
    ret = Hash.new(0)
    trimmed.each do |n|
  ...
  def get_in_preposition(tagged)
    return nil unless valid_text(tagged)
    IN
    trimmed = tagged.scan(IN).map do |n|
      strip_tags(n)
    end
    ret = Hash.new(0)
    trimmed.each do |n|
  ...
  def get_to_preposition(tagged)
    return nil unless valid_text(tagged)
    TO
    trimmed = tagged.scan(TO).map do |n|
      strip_tags(n)
    end
    ret = Hash.new(0)
    trimmed.each do |n|
  ...
  def get_wp_pronoun(tagged)
    return nil unless valid_text(tagged)
    WP
    trimmed = tagged.scan(WP).map do |n|
      strip_tags(n)
    end
    ret = Hash.new(0)
    trimmed.each do |n|
  ...
  def get_ex_pronoun(tagged)
    return nil unless valid_text(tagged)
    EX
    trimmed = tagged.scan(EX).map do |n|
      strip_tags(n)
    end
    ret = Hash.new(0)
    trimmed.each do |n|
  ...
  def get_wps_determiners(tagged)
    return nil unless valid_text(tagged)
    WPS
    trimmed = tagged.scan(WPS).map do |n|
      strip_tags(n)
    end
    ret = Hash.new(0)
    trimmed.each do |n|
  ...
  def get_wdt_determiners(tagged)
    return nil unless valid_text(tagged)
    WDT
    trimmed = tagged.scan(WDT).map do |n|
      strip_tags(n)
    end
    ret = Hash.new(0)
    trimmed.each do |n|
  ...
  def get_pdt_determiners(tagged)
    return nil unless valid_text(tagged)
    PDT
    trimmed = tagged.scan(PDT).map do |n|
      strip_tags(n)
    end
    ret = Hash.new(0)
    trimmed.each do |n|
  ...
  def get_prp_determiners(tagged)
    return nil unless valid_text(tagged)
    PRP
  ...
  def get_prps_determiners(tagged)
    return nil unless valid_text(tagged)
    PRPS
    trimmed = tagged.scan(PRPS).map do |n|
      strip_tags(n)
    end
    ret = Hash.new(0)
    trimmed.each do |n|
  ...
  def get_pure_determiners(tagged)
    return nil unless valid_text(tagged)
    DET
    trimmed = tagged.scan(DET).map do |n|
      strip_tags(n)
    end
    ret = Hash.new(0)
    trimmed.each do |n|
